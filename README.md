...
```
quickPatterns.layer(0).addPattern(new qpBouncingPulse(8)); //add a pattern directly to layer 0
...
Patterns can also be displayed using colors chosen from a palette, either sequentially or randomly.

To configure a pattern to cycle through palette colors sequentially, use the *chooseColorSequentiallyFromPalette()* method and pass in any CRGBPalette16
```
...
Both chooseColorSequentiallyFromPalette() and chooseColorRandomlyFromPalette() can accept a second optional parameter, the hue step, that sets the size of change when moving through the palette hues.
...
quickPatterns.newScene().addPattern(new qpBouncingPulse(8))
  .singleColor(CRGB::Red);
```
...
quickPatterns.scene(1).addPattern(new qpBouncingPulse(8))
  .singleColor(CRGB::Red);
```
...
quickPatterns.scene(2).layer(1).addPattern(new qpBouncingPulse(8))
  .singleColor(CRGB::Red);
```
...
quickPatterns.sameScene().addPattern(new qpBouncingPulse(8)) //creates layer 1, pattern 0 on the same scene that was just created
...
i.e.
```
quickPatterns.scene(0).layer(1).addPattern(new qpBouncingPulse(8));
...
Once written, include your pattern in your sketch and you can now add your pattern to be displayed like so
```
